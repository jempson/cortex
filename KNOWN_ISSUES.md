...
The playback feature (timeline slider that reveals pings chronologically) has some edge cases that need future work:

1. **Occasionally missed pings**: In long waves (50+ pings), a small number of pings may not be highlighted/scrolled to during playback. The `_index` assignment uses chronological order based on `created_at`, but edge cases exist where:
   - Pings with identical timestamps may have unpredictable ordering
   - Deeply nested reply threads loaded via "Load more" may have indexing gaps

2. **Scroll position after loading older pings**: When "Load older pings" is triggered during playback, the scroll position calculation may not perfectly align with the current playback position.
...
**Workaround**: For complete chronological viewing, use the wave normally without playback mode. Playback works reliably for waves with fewer than 50 pings.

**Technical details**:
- Playback uses `_index` assigned in chronological order from `created_at` timestamps
- All pings are loaded before playback starts via `handlePlaybackToggle`
- Scroll uses viewport-relative positioning via `getBoundingClientRect()`
- See `FarholdApp.jsx` lines ~8800-8900 for playback implementation
